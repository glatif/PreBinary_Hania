...
CREATE TABLE practice_quiz_attempts (
    ...
    -- taking the quiz (see section 21 below). NULL for attempts taken before
    -- this feature existed, or for non-student preview attempts.
...
--
-- This data is intentionally lightweight and short-lived — it cascades away
-- with the user/quiz/assessment it describes rather than being retained
-- independently.
...
-- Deleting a row here does not delete the file on disk; only the DB record
-- cascades away with the user/quiz/assessment it references.
